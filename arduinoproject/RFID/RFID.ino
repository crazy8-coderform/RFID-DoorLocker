...
const int NRSTPD = 5;
const int LED_PIN=2;
const int DOOR_CLOSE=3;
/////////////////钥匙参数////////////////////////////////////
unsigned char Key_buf[5];
//unsigned char Key_Test[5]={0x93,0xd3,0x45,0x8c,0x89};//钥匙密码（也就是卡号）
unsigned char Key_Test[5]={};
unsigned char Key_Flag=0;
bool first_flag = false;      //第一次上电的标识
...
{                
    ...
    lcd.print("Please Use Card ");     //输出字符到LCD1602上
    lcd.setCursor(0,1);  
    lcd.print("  To Initiated  ");
    first_flag = true;
    myservo.write(180);
    delay(1000);     
}
...
void loop()
{
    ...
    int close_flag = HIGH;
    close_flag = digitalRead(DOOR_CLOSE);
    if(LOW == close_flag && door_flag == true)
    {
        /*关门程序*/
        myservo.write(180);
        close_flag = HIGH;
    ...
    memcpy(serNum, str, 5);
    if(first_flag == true && status == MI_OK)
    {
        memcpy(Key_Test, serNum, 5);
        first_flag = false;
        lcd.setCursor(0,1);
        lcd.print("                ");
        Serial.println("!!!!!!!!!!!!!!!!");
    }
    else if(door_flag == false && first_flag == false && door_flag == false)
    {
        lcd.setCursor(0,0);
    ...
    if (status == MI_OK && door_flag == false)
    {
        ...
        Serial.println(" ");
        Serial.println("KEYBUF NUMBER IS : ");
        Serial.print(Key_buf[0],HEX);
        Serial.println(" ");
        Serial.print(Key_buf[1],HEX);
        Serial.println(" ");
        Serial.print(Key_buf[2],HEX);
        Serial.println(" ");  
        Serial.print(Key_buf[3],HEX);
        Serial.println(" ");
        Serial.print(Key_buf[4],HEX);
        Serial.println(" ");
        Serial.println("KEYTEST NUMBER IS : ");
        Serial.print(Key_Test[0],HEX);
        Serial.println(" ");
        Serial.print(Key_Test[1],HEX);
        Serial.println(" ");
        Serial.print(Key_Test[2],HEX);
        Serial.println(" ");  
        Serial.print(Key_Test[3],HEX);
        Serial.println(" ");
        Serial.print(Key_Test[4],HEX);
        Serial.println(" ");
        OPPEN_Door();       
        delay(500);
    }
